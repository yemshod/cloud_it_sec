...
1.  **External Interactor (User):** The source of the prompt. Threats: **Spoofing** (LLM01).
2.  **Process (Application Backend):** Where input/output handling and logic reside. Threats: **Tampering** (LLM02), **Information Disclosure** (LLM06).
3.  **Process (LLM Model/API):** The AI component itself. Threats: **Denial of Service** (LLM04), **Model Theft** (LLM10).
4.  **Data Store (Training/RAG Data):** Source of knowledge. Threats: **Tampering** (LLM03).
5.  **External Tool/Plugin:** Any system the LLM can execute code on. Threats: **Elevation of Privilege** (LLM07, LLM08).
6.  **Trust Boundary:** The red line in the DFD where security controls **must** be enforced (e.g., between the application backend and the LLM API). This is where most LLM-specific vulnerabilities reside.
