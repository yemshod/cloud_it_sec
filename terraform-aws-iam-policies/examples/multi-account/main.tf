/**
 * # Multi-Account IAM Policies Deployment Example
 *
 * This example demonstrates how to deploy IAM policies and permission boundaries
 * across multiple AWS accounts using Terraform workspaces.
 */

provider "aws" {
  region = var.region
  
  # Use assume_role for cross-account deployment
  assume_role {
    role_arn = "arn:aws:iam::${var.account_ids[terraform.workspace]}:role/OrganizationAccountAccessRole"
  }
}
